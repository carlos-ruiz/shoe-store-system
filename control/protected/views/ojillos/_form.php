			...
			<div class="col-md-4">
				...
				<?php 
					...
					foreach ($model->ojillosColores as $ojilloColor) {
						array_push($coloresSeleccionados, $ojilloColor->color);
					}
				?>
				<?php foreach ($colores as $color) { ?>
				<div class="form-group col-md-6">
					<label for="OjillosColores_id_colores_<?= $color->id ?>"><?= $color->color; ?></label>
					<div class="input-group inline-block">
						<input size="45" maxlength="45" name="OjillosColores[id_colores][<?= $color->id; ?>]" id="OjillosColores_id_colores_<?= $color->id ?>" value="1" type="checkbox" <?php if(in_array($color, $coloresSeleccionados)){echo "checked";} ?>>
					</div>
				</div>
				<?php } ?>
			</div>
