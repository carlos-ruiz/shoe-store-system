	...
	<div class="form-body">
		...
		<div class="row">
			<div class="panel panel-red panel-ordenes">
				<div class="panel-heading"><?= (isset($accion) && $accion=='descontar')?"Descontar tacones del inventario":"Agregar tacones a inventario" ?></div>
								...
								<tr id="row_<?= $time ?>">
									...
									<?php for ($i=12; $i < 32; $i++) { ?>
										...
										<?php if(in_array($i, $numerosPosibles)) {?>
											<input class="input-cantidad" type="text" title="<?= $i ?>" name="Inventario[numeros][<?= $time ?>][<?= $i ?>]" size="1" <?php echo "value='";if(isset($row[''.$i])){echo $row[''.$i];}echo "'"; ?>/>
										<?php } 
										else { echo " - ";}?>
