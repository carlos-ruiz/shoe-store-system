...
class PedidosController extends Controller {
                ...
                if ($model->validate() && $model->save()) {
                    ...
                    if ($model->id_estatus_pedidos == $estatusPedidoEnProceso->id) {
                        ...
                        if ($errores == 'true') {
                            ...
                            $model->save();
                            $titulo = 'Aviso';
                            $mensaje = 'No hay suficiente material para completar el pedido';
    ...
    public function actionAdmin() {
        ...
        $pedidos = filter_input(INPUT_GET, 'Pedidos');
        if (isset($pedidos)) {
            $model->attributes = $pedidos;
        }
    ...
    public function actionAgregarOrden() {
        if (isset($_POST)) {
            ...
            <?php for ($i = 12; $i < 32; $i = $i + 0.5) { ?>
                    <td data-numero="<?= $i; ?>">
                        <input class="input-cantidad" type="text" name="Pedido[numeros][<?= $time ?>][<?= $i; ?>]" title="<?= $i ?>" maxlength="3" style="width:20px;" <?php if (!in_array($i, $numerosPosibles)) {
                    echo "disabled value='X'";
                } else {
                    echo "value=''";
                } ?>/>
                    </td>
...
// Fin metodo apartarMateriales
    ...
    public function actionEmpezarPedido($id) {
        ...
        if ($pedido->estatus->nombre === 'Pendiente') {
            $errores = $this->actualizarInventario($id);
            if ($errores == 'true') {
                $titulo = 'Aviso';
                $mensaje = 'No hay suficientes materiales para cubrir el pedido';
            } else {
